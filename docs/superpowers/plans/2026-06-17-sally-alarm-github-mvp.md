...
**Files:**
- Create: `src/assets/tray-Template.png` (발바닥 template 아이콘 — `src-tauri/icons/tray-Template@2x.png`를 복사)
...
**아이콘 방식 (확정):** 메뉴바 트레이 아이콘은 발바닥 단색 template PNG다. `defaultWindowIcon()`(풀컬러 앱 아이콘)을 쓰지 않는다. 프론트엔드 에셋으로 PNG를 import해 바이트로 읽어 `TrayIcon.new`의 `icon`(타입: `string | Uint8Array | ArrayBuffer | number[] | Image`)에 넘기고, `iconAsTemplate: true`로 macOS가 라이트/다크 메뉴바에 맞춰 자동 렌더하게 한다. PNG 디코딩은 `Cargo.toml`의 `image-png` feature가 담당한다.

- [ ] **Step 1: 트레이 아이콘 에셋 준비**

```bash
mkdir -p src/assets
cp src-tauri/icons/tray-Template@2x.png src/assets/tray-Template.png
```

(`src/vite-env.d.ts`의 `vite/client` 참조가 `*.png` import를 string URL로 선언하므로 별도 타입 선언 불필요.)
...
설치된 `node_modules/@tauri-apps/api/tray.d.ts`의 `TrayIconOptions`로 옵션명(`icon`/`iconAsTemplate`/`menuOnLeftClick`/`action`/`tooltip`)과 `TrayIconEvent`의 판별자(`event.type`)를 확인하며 작성한다.
...
```bash
git add src/assets/tray-Template.png src/app/tray.ts
git commit -m "feat: 메뉴바 발바닥 template 트레이 아이콘 및 메뉴 구성"
```
